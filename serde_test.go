...
func (s *Suite) TestSerdeFunctionPerformance(assert *require.Assertions, L *lua.State) {
	L.PushGoFunction(ltask.LuaSerdePack)
	L.SetGlobal("pack")
	L.PushGoFunction(ltask.LuaSerdeUnpack)
	L.SetGlobal("unpack")
...
for i = 1, 100 do
	...
	for j = 0, 9 do
		local func_name = "test_func_" .. j
		local ptr_name = "test_ptr_" .. j
		big_data[i].funcs[j] = _G[func_name]
		big_data[i].ptrs[j] = _G[ptr_name]
	end
end

local msg, sz = pack(big_data)
local pack_time = os.clock()

local result = unpack(msg)
local unpack_time = os.clock()

assert(#result == 100)
assert(type(result[1].funcs[0]) == "function")
assert(type(result[1].ptrs[0]) == "userdata")
assert(result[50].funcs[5]() == 5)

print(string.format("Function/Pointer Pack time: %.3fs, Unpack time: %.3fs", 
	pack_time - start_time, unpack_time - pack_time))
	`)
	assert.NoError(err)
}
