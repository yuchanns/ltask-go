-- copy from https://github.com/ejoy/ant/blob/master/tools/fileserver/pkg/s/service/network.lua
...
local function fd_update(s)
  local flags = 0
  if s.r then flags = flags | EPOLLIN end
  if s.w then flags = flags | EPOLLOUT end
...
local function close(s)
  ...
  assert(s.shutdown_r)
  assert(s.shutdown_w)
  if s.wait_read then assert(#s.wait_read == 0) end
  if s.wait_write then assert(#s.wait_write == 0) end
...
local function stream_on_read(s)
  ...
  elseif data == false then
    ...
    if #s.readbuf > kMaxReadBufSize then fd_clr_read(s) end
  end
end
...
function S.connect(protocol, ...)
  ...
  if r == nil then return r, err end
  local s = {
    fd = fd,
    wait_write = {},
    shutdown_r = true,
...
function S.listen(h)
  ...
  if not newfd then return nil, err end
...
function S.send(h, data)
  ...
  if s.shutdown_w then return end
  if data == "" then return 0 end
  if #s.wait_write == 0 then fd_set_write(s) end
...
function S.recv(h, n)
  ...
  if s.shutdown_r then
    if not n then
      if s.readbuf == "" then return end
    else
      if n > kMaxReadBufSize then n = kMaxReadBufSize end
      if n > #s.readbuf then return end
    end
  end
  local sz = #s.readbuf
  if not n then
    ...
    local ret = s.readbuf
    if sz > kMaxReadBufSize then fd_set_read(s) end
    s.readbuf = ""
    return ret
  else
    if n > kMaxReadBufSize then n = kMaxReadBufSize end
    if n <= sz then
      ...
      if sz > kMaxReadBufSize and sz - n <= kMaxReadBufSize then fd_set_read(s) end
      s.readbuf = s.readbuf:sub(n + 1)
      return ret
    else
      local token = {
...
ltask.idle_handler(function()
  for fd, event in epfd:wait() do
    if event & (EPOLLERR | EPOLLHUP) ~= 0 then event = event & (EPOLLIN | EPOLLOUT) end
