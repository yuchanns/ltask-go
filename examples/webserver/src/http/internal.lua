...
local function chunksize(readbytes, body)
  while true do
    local f, e = body:find("\r\n", 1, true)
    if f then return tonumber(body:sub(1, f - 1), 16), body:sub(e + 1) end
    if #body > 128 then
...
function M.recvchunkedbody(readbytes, bodylimit, header, body)
  ...
  while true do
    ...
    size = size + sz
    if bodylimit and size > bodylimit then return end
    if #body >= sz then
...
function M.request(interface, method, host, url, recvheader, header, content)
  ...
  if header then
    if not header.Host then header.Host = host end
  ...
  if not body then error("Recv header failed") end
  ...
  if not header then error("Invalid HTTP response header") end
...
function M.response(interface, code, body, header)
  ...
  if mode then
    if mode ~= "identity" and mode ~= "chunked" then error("Unsupport transfer-encoding") end
  end

  if mode == "chunked" then
    body, header = M.recvchunkedbody(interface.read, nil, header, body)
    if not body then error("Invalid response body") end
  else
    -- identity mode
...
function stream:padding() return self._reading(self), self end
...
local function stream_length(length)
  return function(stream)
    ...
    if n >= length then
      ...
      length = length - n
      stream._body = nil
      if not stream.connected then stream._reading = stream.close end
      return body
    end
  end
end
